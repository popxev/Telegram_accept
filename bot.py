...
async def help_command(update: Update, context: CallbackContext): help_text = """أوامر البوت: /start - بدء البوت /help - عرض الأوامر /contact - للتواصل معنا /discord - للانضمام إلى سيرفر ديسكورد"""

await update.message.reply_text(help_text)

async def contact(update: Update, context: CallbackContext): contact_text = """للتواصل معنا، يمكنك زيارة قنواتنا الرسمية: يوتيوب: Popxev Games إنستجرام: Popxev Games فيسبوك: Popxev Games ديسكورد: انضم إلينا"""
...
@app.route("/webhook", methods=["POST"]) def webhook(): update = Update.de_json(request.get_json(), application.bot) asyncio.run(application.process_update(update)) return "OK", 200

def main(): application.add_handler(CommandHandler("start", start)) application.add_handler(CommandHandler("help", help_command)) application.add_handler(CommandHandler("contact", contact)) application.add_handler(CommandHandler("discord", discord)) application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_messages))
